...
// Removed MotionNavLink

export const Sidebar = ({ onNewLead, onExport }: SidebarProps) => {
  const { user, isAdmin, logout } = useAuth();

  return (
    <motion.nav
      initial={{ x: -80, opacity: 0, width: 72 }}
      animate={{ x: 0, opacity: 1, width: 72 }}
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto py-2 px-2 space-y-1">
        {navItems.map((item, i) => (
          ...
          >
            <NavLink
              to={item.path}
              className={({ isActive }) => `flex items-center px-3 py-2.5 rounded-lg transition-all duration-200 group/item
                ${isActive
        ...
        {isAdmin && (
          <motion.button
            ...
            className="flex items-center px-3 py-2.5 w-full rounded-lg text-on-surface-variant hover:text-on-surface hover:bg-surface-variant/30 transition-all duration-200"
          >
      ...
      <div className="p-3 shrink-0 border-t border-outline-variant/10 space-y-2">
        ...
        {/* User info */}
        <NavLink to="/settings" className="flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-surface-variant/30 transition-colors">
